...
func (s *Servers) NextIndex() int {
	log.Printf("NextIndex: Backend[%x] -> Backend[%d] ", (s.Current)%uint64(len(s.Backends)), (s.Current+1)%uint64(len(s.Backends)))
...
func (s *Servers) StateCheck() {
	for _, b := range s.Backends {
		...
		if err != nil {
			b.SetAlive(false)
			log.Println("StateCheck: "+b.Addr.String()+"is not working because", err)
		} else {
			b.SetAlive(true)
